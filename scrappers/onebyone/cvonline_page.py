...
for x in tqdm(range(0,data_size)):
    ...
    with open(f'../txt/cvonline/0/soup/soup{x}.html','w') as f:
        f.write(full_page.prettify())
    with open(f"../txt/cvonline/0/temp.txt" , "w") as f:
        f.write(full_page.text)
    with open(f"../txt/cvonline/0/temp.txt" , "r") as f:
        for line in f:
            if(line.strip()):
    ...
    with open(f"../txt/cvonline/0/job{x}.txt" , "w") as f:
        f.write(text)


    text = ""
    os.remove("../txt/cvonline/0/temp.txt")
    cond = False
